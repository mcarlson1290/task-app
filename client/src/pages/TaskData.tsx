...
const TaskData: React.FC = () => {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (filters.checklistCompletionRate !== 'all') {
      filtered = filtered.filter(task => {
        ...
        const rate = (completedItems / task.checklist.length) * 100;
  ...
  const handleExport = () => {
    const csvContent = [
      ['Task', 'Type', 'Status', 'Priority', 'Assigned To', 'Progress', 'Estimated Time', 'Actual Time', 'Due Date', 'Created At'],
      ...filteredTasks.map(task => [
        ...
        task.actualTime ? `${task.actualTime}m` : 'N/A',
        new Date(task.dueDate).toLocaleDateString(),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#203B17] mb-2">Task Data Analytics</h1>
          <p className="text-gray-600">Comprehensive task performance and analytics dashboard</p>
        </div>
      ...
      {/* Filters */}
      <TaskDataFilters filters={filters} setFilters={setFilters} tasks={tasks} />
